...
def project_minor_transaction(row: list[str]) -> list[str]:
    """
    Project the output row for minor USD transactions.
    ...
    return [
        ...
        row[TO_ACCOUNT].strip(),
        row[AMOUNT_PAID].strip(),
    ]


def generate_expected_file(input_file: str, expected_file: str) -> None:
    """
    Generate the expected output file for USD transactions below the threshold.

    The generated file has no header and only includes:
    source account, destination account and amount.
    """
...
def main() -> None:
    ...
    if not input_file.exists():
        ...
        generate_expected_file(str(input_file), str(expected_file))
