...
def _recv_transaction_batch(socket):
    count = external_serializer.deserialize_uint32(
        _recv_sized(socket, external_serializer.UINT32_SIZE)
    )
    return [_recv_transaction_record(socket) for _ in range(count)]
...
def _send_transaction_batch(socket, rows):
    msg = external_serializer.serialize_uint32(MsgType.TRANSACTION_BATCH)
    msg += external_serializer.serialize_uint32(len(rows))
    for row in rows:
        [timestamp, from_bank, from_account, to_bank, to_account,
         amount_received, receiving_currency, amount_paid,
         payment_currency, payment_format, is_laundering] = row
