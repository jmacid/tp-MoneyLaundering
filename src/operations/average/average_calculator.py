...
class AverageCalculator(OperationStrategy):
    ...
    def __init__(self, group_by: str | None = None):
        ...
        self.required_fields = {self.avg_group, "amount_paid"} #"client_id", only for demo

        self.stats: dict[str, dict[str, Decimal | int]] = defaultdict(lambda: {"count": 0, "sum": Decimal("0")})
    ...
    def process(self, transaction: dict[str, Any]) -> dict[str, Any] | None:
        ...
        groped_by = transaction[self.avg_group]

        self.count[client_id][groped_by]["count"] += 1
        self.count[client_id][groped_by]["sum"] += transaction["amount_paid"]
